...
> **Статус:** План реализации (в процессе миграции с Supabase)  
> **Инструменты:** PocketBase v0.23+, Brevo (SMTP Relay), Cloudflare Turnstile.
...
### Этап 2: Защита от ботов (Cloudflare Turnstile)
- [ ] Добавить компонент `Turnstile` в форму регистрации.
- [ ] Реализовать проверку токена на бэкенде. В PocketBase это можно сделать через `onRecordBeforeCreateRequest` в `pb_hooks`.
