...
> **Текущий статус (19 августа 2026):** базовые privacy-safe fallback'и активного
> и входящего экранов проверены локально, но этот план остаётся открытым. В
> текущем коде `CallsList` ещё передаёт в `initiateCall` только room/type, а
> `acceptCall` создаёт active session с пустыми `displayName/avatarUrl`; единый
> контекст public/private/group для этих путей не доведён до DoD. После
> реализации потребуются owner-run двухклиентские проверки. Серверный
> call-history `expand` и фактический Dev authorization остаются отдельными
> security-проверками.
