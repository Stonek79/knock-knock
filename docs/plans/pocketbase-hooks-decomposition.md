...
> **Статус:** локальная реализация завершена в коммите `ef77176`: монолит
> удалён, модули `main.01`–`main.08` и helpers проверены static/contract
> тестами. Dev runtime smoke-test двух principals выполняется владельцем;
> этот план не закрывает production migration или release readiness.
...
- Конфликт двух файлов `ADR-0002` зафиксирован в ADR-0004; существующие ADR не
  перезаписываются, а ссылки должны использовать имена файлов.
