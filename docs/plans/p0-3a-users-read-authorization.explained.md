...
Локальная реализация выполнена в коммите `7f142f3` и последующей hook-
декомпозиции: owner-only snapshot, capability DTO, parameter binding и
frontend consumers проверены локальными тестами. Файл с техническим планом
остаётся источником критериев, а этот файл — объяснением для человека.

Открытый шаг — ручная Dev-проверка двумя обычными пользователями после
применения правил через Admin UI. До неё P0.3a и release readiness не считаются
полностью закрытыми; миграции и остальные P0/P1 находятся за пределами этого
среза.
