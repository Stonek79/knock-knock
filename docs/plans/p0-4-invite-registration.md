...
владелец применяет snapshot в Dev и выполняет valid, expired,
...
- [ ] Dev snapshot применён и фактические rules проверены владельцем;
...
- [ ] **CONCURRENCY RUNTIME GATE**: закрепить версию PocketBase, применить
  snapshot/hooks в Dev и прогнать двухпользовательскую concurrent matrix без
