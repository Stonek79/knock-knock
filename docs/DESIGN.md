...
> Рабочий стандарт оформления. Перед крупным рефакторингом сверить tokens и
> компоненты с текущим кодом: связанный `DESIGN_SYSTEM_PLAN.md` частично
> исторический и не является подтверждённым checklist реализации.
...
@radix-ui                 ← используется ТОЛЬКО внутри components/ui/
      ↓
styles/themes/*.css       ← источник цветовых токенов и тем
```
...
- **Глобальные токены**: `app/src/styles/themes/base.css` содержит базовые токены,
  а `app/src/styles/themes/*.css` — значения цветовых тем (`[data-theme="..."]`).
...
Все цвета в потребляющем коде — CSS-переменные из `app/src/styles/themes/`;
значения тем определяются только в файлах тем и переключаются через `data-theme` и `data-mode`.
...
| `--shadow-badge` | Тень компактных статусных бейджей |

В компонентах и CSS-модулях запрещены литеральные HEX/RGB-цвета и именованные
цвета (`black`, `white`, `gray` и т. п.). Литеральные значения допускаются только
в самих файлах определения тем и базовых токенов.
...
## 👆 Hit-area малых иконок и единый механизм расширения

Размеры меньше `--size-touch` (40px) расширяют кликабельную область до этого токена
через невидимый центрированный `::before` поверх фона кнопки. Визуальные размеры и
фон не меняются, семантическая кнопка, `:focus-visible`, disabled и клавиатурная
навигация остаются прежними.

- `IconButton` (размеры `xs`/`sm`/`md`) и `Button` (размер `icon`) получают
  атрибут `data-hit-area=<size>`, который включает механизм.
- Текстовые кнопки `Button` (`xs`/`sm`/`md`) **не** расширяются: рост высоты через
  `::before` перекрывал бы соседние интерактивные контролы в плотных
  stacked-раскладках (например, слайдер прогресса аудио-плеера).
- В плотное соседство можно явно выключить расширение: `data-hit-area="none"`.

## 🖼️ Пропорции медиа-галереи

`aspect-ratio` медиагалереи централизован в локальной переменной `--gallery-ratio`,
задаваемой один раз на ячейке сетки (`data-count="2/3/4/many"`) и переиспользуемой
и для `.attachmentImage`, и для `.imagePlaceholder`. Одиночное медиа (`data-count="1"`)
и `.ratio_*` классы для плейсхолдеров остаются изолированной локальной схемой.
...
> **Статус:** рабочее направление; фактическое покрытие требует повторного
> аудита. Историческая спецификация: `DESIGN_SYSTEM_PLAN.md`.
