...
> **Приоритет:** Этап 0 в `IMPLEMENTATION_PLAN_v2.md` — выполняется перед Auth
...
> ⬜ **Статус:** Не реализуется сейчас — зафиксировано как техдолг

### Концепция: Мета-токены (двухуровневая система)

Текущая система (v2.x) работает напрямую: компонент → CSS-переменная темы.

Следующий уровень (v3) вводит **семантические мета-токены**, которые не зависят от конкретной темы:

```css
/* Мета-токены (не зависят от темы): */
--color-text-primary:       /* основной текст */
--color-text-secondary:     /* вторичный текст */
--color-surface-default:    /* поверхность */
--color-surface-elevated:   /* приподнятая поверхность */
--color-action:             /* интерактивный цвет */
--color-action-hover:       /* hover */
--color-separator:          /* разделители */

/* Каждая тема задаёт значения мета-токенов: */
...
- Рефакторинг всех CSS-модулей компонентов (замена `--muted` → `--color-text-secondary` и т.д.)
- Единое именование без дублей (сейчас `--muted` vs `--muted-foreground` vs `--gray-11`)
- Полная изоляция компонентов от конкретных тем

### Когда переходить на v3:

После стабилизации Auth + SMTP этапов. Оптимально — до начала разработки Calls (чтобы новые компоненты сразу писались по v3).
