...
> Статус: 🟢 Реализовано (Staging Ready)
...
- [x] Разделение .env файлов (.test, .production)
- [x] Настройка Staging Supabase (VPS + домашний сервер)

### В процессе (In Progress - План А)
- [ ] Playwright E2E тесты для всех фич (Auth, Chat, Crypto, Media)
- [ ] Integration тесты (Vitest + Real DB)
- [ ] Фикстуры и хелперы для тестов (db-cleanup)
- [ ] Аудит документации и JSDoc (Этап 4 Плана А)

### Запланировано (Planned - План Б)
- [ ] Страница правил (`/terms`) и онбординг
- [ ] Форма обратной связи в настройках
- [ ] CI/CD пайплайн (GitHub Actions)
- [ ] Отчеты о покрытии тестами (> 40%)
