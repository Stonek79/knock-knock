...
2.  **XSS Protection**: Не используй `dangerouslySetInnerHTML` для рендеринга сообщений.
3.  **Storage**: Ключи хранятся в `IndexedDB` (не LocalStorage) с параметром `non-extractable` (по возможности).
