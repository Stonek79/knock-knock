...
Проект организован по функциональным доменам (Features), а не по типу файлов.

```
app/src/
├── features/           # 🟢 БИЗНЕС-ЛОГИКА (Домены)
│   ├── chat/           # Всё, что касается чата (списки, окна, инпуты)
...
3.  **Layouts** управляют расположением **Features** на странице.
4.  **Lib** — это ядро, без зависимостей от React (по возможности).
...
1.  **Radix First**: Используй компоненты Radix Themes (`<Flex>`, `<Box>`, `<Text>`) для верстки. Не пиши `div` с классами для лейаута.
...
## 🔐 Безопасность (Security Guidelines)

1.  **Secret Keys**: Приватный ключ (`cryptoKey`) **никогда** не должен покидать клиент (браузер). Никаких отправок на сервер.
2.  **XSS Protection**: Не используй `dangerouslySetInnerHTML`. Настроен CSP (`Content-Security-Policy`).
3.  **Storage**: Ключи хранятся в `IndexedDB` (не LocalStorage) с параметром `non-extractable`.
4.  **Rate Limiting**: Блокировка действий на клиенте после 5 неудачных попыток (защита от Brute-force).

---

## 🛡 Security Testing
Ежемесячно проводить проверку на:
-   **XSS**: Попытка инъекции `<script>` в поля ввода.
-   **Brute-Force**: Проверка блокировки после N попыток входа.
-   **Dependency Audit**: `npm audit` для поиска уязвимостей.
