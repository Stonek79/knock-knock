...
> ⚠️ Операционная заметка для старого домена. Адреса, отправитель и SMTP
> параметры здесь не являются публичными шаблонами; сверяйте их с текущим
> `.env` и настройками PocketBase.

> **Статус:** ✅ Актуальный базовый стандарт почтовой системы
> **Актуальная документация:** См. `AUTH_STRATEGY.md` и `CURRENT_STATE.md`.
