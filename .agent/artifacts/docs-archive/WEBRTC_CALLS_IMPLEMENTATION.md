...
> ⚠️ Исторический проектный документ. В нём остались старые варианты на базе
> Supabase/Edge Functions и старые домены. Текущий LiveKit runtime и проверки
> описываются в `TAURY_PLAN.md` и `CURRENT_STATE.md`.
...
### Выбранное решение: LiveKit Server (Self-Hosted на whoami.ninja)

**Обоснование выбора:**

| Критерий | Преимущество |
|----------|--------------|
| **Приватность (ZK)** | Анонимизированные идентичности (`anon_${md5}`), шифрованные логи `encrypted_metadata` |
| **Собственный UI-Кит** | 100% кастомный UI на компонентах `src/components/ui/` (без сторонней верстки) |
| **PIP-сворачивание** | Плавающий виджет `CallPIPWidget.tsx` для сохранения доступа к чатам при звонке |
| **Темизация** | Полная поддержка встроенных (`default`, `neon`, `emerald`) и Telegram-style тем |
| **Инфраструктура** | Размещение на VPS  с проксированием WSS |
...
│  │  ├─ CallRoom, CallControls, CallPIPWidget           │   │
│  │  ├─ useCallStore (Zustand) + PIP                    │   │
│  │  └─ Разграничение: Открытый / Инкогнито профиль     │   │
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
                          ↕ (wss://whoami.ninja/livekit/)
              ┌───────────────────────────────┐
              │  LiveKit Server (whoami-livekit)│
              └───────────────────────────────┘
              └───────────────────────────────┘
                          ↕ (WebSocket + UDP)
┌─────────────────────────────────────────────────────────────┐
