# Глобальный план развития Nemo

> ⚠️ Исторический план. Часть пунктов отмечена как выполненная без текущего
> runtime-подтверждения, а отдельные разделы ссылаются на старую архитектуру.
> Актуальные решения находятся в `CURRENT_STATE.md`, `prod_readiness_plan.md`
> и `TAURY_PLAN.md`.
