# План миграции на Cloudflare Tunnels (ПЛАН ПРОВАЛЕН)

> **ВНИМАНИЕ: Данный план был признан нерабочим.**
> Попытка использовать `cloudflared` (Cloudflare Tunnel) потерпела неудачу из-за систем ТСПУ (DPI) провайдеров в РФ. Провайдеры научились определять специфический отпечаток TLS (SNI) туннелей Cloudflare и жестко обрывают соединение (ошибка `TLS handshake with edge error: EOF`) прямо в момент рукопожатия, даже если трафик заворачивается в сторонний VPN (например, Hiddify).
> 
> **Вместо этого реализована и работает архитектура:** FRP (Fast Reverse Proxy) поверх Hiddify VPN (happ) + Nginx на VPS. Подробности в `docs/BYPASS_STRATEGY.md`.
