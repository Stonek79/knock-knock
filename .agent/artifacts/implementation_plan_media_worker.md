...
- [x] Отправить изображение → мгновенно появляется из кэша (blob URL)
- [x] Открыть Lightbox → изображение корректное, не зашифрованное
- [x] Перезагрузить страницу → изображение грузится из Dexie, не с сервера
- [x] Отправить документ → скачивается расшифрованный файл
- [x] Голосовое сообщение → воспроизводится, плеер не показывает ошибку
- [x] Мультиселект 2 видео → показывается ошибка валидации
- [x] Медленная сеть → скелетон без UI freeze
