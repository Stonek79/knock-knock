---
name: architect
description: Эксперт по чистому коду, TypeScript и архитектуре проекта Nemo. Используйте этот навык для рефакторинга, создания новых модулей, проверки типов и обеспечения модульной структуры.
---
...
Перед анализом прочитай корневой `AGENTS.md`, `docs/CURRENT_STATE.md` и
`docs/ARCHITECTURE_AUDIT.md`. Эти документы важнее старых примеров в skill.
...
2.  **React Standards**:
    *   Используй API фактически установленной версии React и проверяй поддержку в текущих библиотеках.
    *   Проверяй код на отсутствие устаревших паттернов и заменяй их на новые стандарты (например, избавляйся от лишних обёрток).
...
6.  **Clean Code & Feature-Driven Architecture (FDA)**:
    *   Функции должны быть небольшими и делать одну вещь.
    *   Мы не используем строгий FSD. Мы группируем код по логическим бизнес-модулям (под-доменам) внутри папки `features/`.
    *   Хуки и UI-компоненты, относящиеся к одному под-домену (например, `modules/Message`), должны лежать рядом.
    *   Фичи (`features/*`) не должны зависеть друг от друга.
...
6.  **Sibling Imports**: Импорт из одной фичи в другую напрямую. Только через общие сторы, `shared` или `AppLayout`.
7.  **50: **Braces for Return**: Любой `return` внутри блока `if` должен быть всегда обернут в фигурные скобки. Однострочные `if (...) return ...` запрещены.
    Пример: `if (!value) { return ok(null); }`
51: **Hardcoded CSS Styles**: Использование сырых пикселей (`px`) и HEX/RGB цветов вместо токенов дизайн-системы (`var(--space-...)`, `var(--font-size-...)`, `var(--radius-...)`, `var(--accent-primary)` и т.д.) в `.css` / `.module.css` файлах.
