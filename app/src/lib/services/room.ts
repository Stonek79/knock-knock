...
export const RoomService = {
	...
	async createRoom(
		...
		for (const profile of profiles) {
			...
			const serializedKey = JSON.stringify({
				ephemeralPublicKey: arrayBufferToBase64(wrapped.ephemeralPublicKey),
	...
	async deleteRoom(roomId: string) {
		...
		await supabase.from(DB_TABLES.ROOM_MEMBERS).delete().eq("room_id", roomId);
		...
		if (error) {
			logger.warn("Could not delete room record (possbily restricted)", error);
		}
	},
