...
export function useMessageSubscription({
		...
		async (payload: RealtimePostgresChangesPayload<MessageRow>) => {
			...
			try {
				...
				if (userId && newMsgRaw.sender_id !== userId) {
					MessageService.markMessageAsDelivered(newMsgRaw.id).catch((err) => {
			...
			try {
				...
				queryClient.setQueryData(
					...
					(old: DecryptedMessageWithProfile[] | undefined) => {
						...
						if (newMsgRaw.is_deleted && newMsgRaw.sender_id === userId) {
	...
	useEffect(() => {
		...
		const channel = supabase
			...
			.subscribe((status) => {
				console.log("Realtime Subscription Status:", status, "Room:", roomId); // DEBUG LOG
			});
