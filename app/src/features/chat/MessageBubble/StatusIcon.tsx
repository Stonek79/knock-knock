...
export function StatusIcon({ status, isOwn, isDeleted }: StatusIconProps) {
	...
	switch (status) {
		case MESSAGE_STATUS.SENT:
			return <Check className={`${iconClass} ${styles.statusIconSent}`} />;
		case MESSAGE_STATUS.DELIVERED:
			return <CheckCheck className={`${iconClass} ${styles.statusIconSent}`} />;
		case MESSAGE_STATUS.READ:
			return <CheckCheck className={`${iconClass} ${styles.statusIconRead}`} />;
		default:
			return null;
	}
}
