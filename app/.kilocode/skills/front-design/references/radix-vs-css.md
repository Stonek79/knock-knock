...
В проекте Knock-Knock мы сочетаем мощь Radix UI примитивов для структуры и гибкость CSS Modules для уникального дизайна.

## Когда использовать Radix UI Props
Используйте встроенные пропсы Radix (`p`, `m`, `gap`, `direction`, `align`, `justify`), если задача стандартная:
- Выравнивание элементов во флекс-контейнере.
- Системные отступы из сетки (1-9).
- Изменение размеров текста через `size`.
...
- Нужно переопределить глубокие стили компонентов Radix (через селекторы).
...
1. **Layout = Radix**. Флексы, гриды и базовые контейнеры делаем через Radix компоненты.
...
## Как НЕ надо делать
- Плохо: `<div style={{ display: 'flex' }}>` -> Используй `<Flex>`.
- Плохо: `<Flex className={styles.flexCenter}>` где `.flexCenter { display: flex; justify-content: center; }` -> Используй `<Flex justify="center">`.
- Плохо: Использовать магические числа `padding: 13px` -> Используй `p="3"` (Radix) или `var(--space-md)`.
